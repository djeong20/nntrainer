...
static void run_int4_gemv_test_(const uint32_t K, const uint32_t N) {
  ...
  std::vector<char> weight =
    generate_random_vector<char, false>(K * N / 2, 0, 255);
  std::vector<uint16_t> scale = generate_random_vector<uint16_t, false>(
    K * N / scale_group_size, -2.0f, 2.0f);
  std::vector<uint16_t> input =
    generate_random_vector<uint16_t, false>(K, -2.0f, 2.0f);
